...
async function syncPendingPedidos(token = null, adminId = null) {
    try {
        ...
        try {
            const respList = await fetch(`${API_URL}/pedidos`, { headers: { 'Authorization': 'Bearer ' + token } });
            if (respList.ok) {
                const serverList = await respList.json();
                const locaisNow = JSON.parse(localStorage.getItem(pedidosKey) || '[]');
                const naoSincronizados = locaisNow.filter(l => l.pendenteSincronizacao || l.pendenteExclusao || l.syncError);

                // Mapeia por serverId/_id e por legacyId para evitar duplicatas
                const map = new Map();
                serverList.forEach(s => {
                    const keyById = s._id ? `id:${s._id}` : null;
                    const keyByLegacy = (s.legacyId != null) ? `legacy:${s.legacyId}` : null;
                    const entry = { ...s, serverId: s._id };
                    if (keyById) map.set(keyById, entry);
                    else if (keyByLegacy) map.set(keyByLegacy, entry);
                });

                naoSincronizados.forEach(ns => {
                    const exists = (
                        (ns.serverId && map.has(`id:${ns.serverId}`)) ||
                        (ns.legacyId != null && map.has(`legacy:${ns.legacyId}`)) ||
                        (ns.id != null && map.has(`legacy:${ns.id}`))
                    );
                    if (!exists) {
                        // Garante que itens locais pendentes sejam mantidos
                        map.set(`local:${Math.random().toString(36).slice(2)}`, ns);
                    }
                });
...
async function syncSinglePedido(elOrId) {
    ...
    if (idx === -1) { showToast('Pedido local não encontrado para sincronizar.', 'error'); return { ok: false, message: 'not-found-local' }; }
        ...
        try {
            ...
            const body = { legacyId: p.id, ...p, dataCriacao: p.dataCriacao || p.id };
            const resp = await fetch(`${API_URL}/pedidos`, { method: 'POST', headers, body: JSON.stringify(body) });
            if (resp.ok) {
...
async function carregarPedidos() {
    ...
    try {
        const resp = await fetch(`${API_URL}/pedidos`, { headers: getAuthHeaders() });
        if (resp.ok) {
            pedidosServidor = await resp.json();
            // Mescla dados do servidor com os locais, preservando flags locais (pendenteSincronizacao, syncError)
            const mergeServerAndLocal = (serverList, localList) => {
                const result = [];
                const usedLocal = new Set();

                for (const s of serverList) {
                    const matchIdx = localList.findIndex(l =>
                        String(l.serverId) === String(s._id) ||
                        String(l._id) === String(s._id) ||
                        (l.legacyId != null && String(l.legacyId) === String(s.legacyId))
                    );

                    if (matchIdx !== -1) {
                        const l = localList[matchIdx];
                        usedLocal.add(matchIdx);
                        const mergedItem = { ...s, serverId: s._id };
                        // Preserve flags from local when present
                        if (l.pendenteSincronizacao) mergedItem.pendenteSincronizacao = true;
                        if (l.syncError) mergedItem.syncError = true;
                        if (l.pendenteExclusao) mergedItem.pendenteExclusao = true;
                        if (l.legacyId != null && mergedItem.legacyId == null) mergedItem.legacyId = l.legacyId;
                        result.push(mergedItem);
                    } else {
                        result.push({ ...s, serverId: s._id });
                    }
                }

                // Adiciona itens locais que ainda não existem no servidor (normalmente pendentes)
                localList.forEach((l, idx) => {
                    if (!usedLocal.has(idx)) {
                        // mantêm somente itens locais que parecem ser pendentes/únicos
                        if (l.pendenteSincronizacao || l.pendenteExclusao || l.syncError || !l.serverId) {
                            result.push(l);
                        }
                    }
                });
            ...
            const merged = mergeServerAndLocal(pedidosServidor, pedidosLocal);
...
async function carregarClientes() {
    ...
    let clientes = [];
    try {
        const resp = await fetch(`${API_URL}/clientes`, { headers: getAuthHeaders() });
        if (resp.ok) {
    ...
    try {
        const resp2 = await fetch(`${API_URL}/clientes`, { headers: getAuthHeaders() });
        if (resp2.ok) {
        ...
        if (adminToken) {
            ...
            (async () => {
                try {
                    ...
                    const resp = await fetch(`${API_URL}/pedidos/${encodeURIComponent(targetId)}`, {
                        method: 'PUT',
                        headers: headersUp,
        ...
        if (adminToken) {
            ...
            (async () => {
                try {
                    ...
                    const resp = await fetch(`${API_URL}/pedidos/${encodeURIComponent(targetId)}`, { method: 'DELETE', headers: { 'Authorization': 'Bearer ' + adminToken } });
                    if (resp.ok) {
                        // remove localmente
...
async function salvarPedido(event) {
    ...
    // Normaliza: vamos primeiro lidar com edição (se houver) e depois
    // criar o novo pedido localmente e tentar um único envio ao servidor.
    ...
    if (idInput && idInput.value) {
        ...
        try {
            ...
            const resp = await fetch(`${API_URL}/pedidos/${targetId}`, {
                method: 'PUT',
                headers: headersUp,
